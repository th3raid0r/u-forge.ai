...
Approved, not implemented. Tool JSON-schema validation and conversation-history
windowing already exist. This plan adds limits around schema injection and the
multi-turn tool loop without weakening useful self-correction.

## Configuration and accounting

- [ ] **AB-01 — Budget configuration.** Add explicit schema-summary,
...
- [ ] **AB-02 — One tokenizer policy.** Reuse the cached tokenizer/estimator for
...
- [ ] **AB-03 — Per-request ledger.** Track consumed and reserved tokens across
  every model/tool turn; expose safe summary fields to tracing and completion
  diagnostics.

## Schema summary selection

- [ ] **AB-04 — Bounded schema prompt.** Enforce the configured schema budget
  before building the Rig agent.
- [ ] **AB-05 — Deterministic prioritization.** Prefer object/edge types named in
...
- [ ] **AB-06 — Honest truncation.** Add a compact notice describing omitted
...
- [ ] **AB-07 — Cumulative stop.** Stop before a model/tool call that cannot fit
...
- [ ] **AB-08 — Repeat fingerprint.** Canonicalize tool name plus validated JSON
...
- [ ] **AB-09 — Progress detection.** Treat new tool arguments, new graph
  mutations, or materially different results as progress. Repeated reads with
  identical output consume the repeat allowance.
- [ ] **AB-10 — Output bounds.** Truncate oversized tool results at semantic
...
- [ ] **AB-11 — Streaming outcome.** Add explicit budget/repeat termination
