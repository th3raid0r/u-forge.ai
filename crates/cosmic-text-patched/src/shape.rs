...
/// Maximum number of ShapePlans to keep in the per-ShapeBuffer cache.
/// Plans are evicted FIFO once this limit is reached.
...
fn shape_fallback(
    ...
    let is_cached = scratch
        .shape_plan_cache
        .iter()
        .any(|(id, dir, scr, _)| *id == font_id && *dir == direction && *scr == script);
    if !is_cached {
    ...
    let shape_plan = &scratch
        .shape_plan_cache
        .iter()
        .find(|(id, dir, scr, _)| *id == font_id && *dir == direction && *scr == script)
