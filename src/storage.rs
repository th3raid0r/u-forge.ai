...
impl KnowledgeGraphStorage {
    ...
    pub fn upsert_edge(&self, edge: Edge) -> Result<()> {
        ...
        let mut from_adj = self.get_adjacency_list(edge.from)?.unwrap_or_else(AdjacencyList::new);
        let mut to_adj = self.get_adjacency_list(edge.to)?.unwrap_or_else(AdjacencyList::new);
        
        // Remove existing edge if it exists (to prevent duplicates)
        from_adj.outgoing.retain(|e| !(e.to == edge.to && e.edge_type == edge.edge_type));
        to_adj.incoming.retain(|e| !(e.from == edge.from && e.edge_type == edge.edge_type));
        ...
        let from_value = bincode::serialize(&from_adj)
            .context("Failed to serialize from adjacency list")?;
        batch.put_cf(cf_edges, from_key, from_value);
        
        let to_key = edge.to.as_bytes();
        let to_value = bincode::serialize(&to_adj)
            .context("Failed to serialize to adjacency list")?;
    ...
    pub fn upsert_chunk(&self, chunk: TextChunk) -> Result<()> {
        ...
        let value = bincode::serialize(&chunk)
            .context("Failed to serialize text chunk")?;
        
        self.db.put_cf(cf_chunks, key, value)
    ...
    pub fn get_stats(&self) -> Result<GraphStats> {
        ...
        let node_count = self.db.iterator_cf(cf_nodes, rocksdb::IteratorMode::Start).count();
        let chunk_count = self.db.iterator_cf(cf_chunks, rocksdb::IteratorMode::Start).count();
    ...
    fn get_adjacency_list(&self, node_id: ObjectId) -> Result<Option<AdjacencyList>> {
        ...
        match self.db.get_cf(cf_edges, key)? {
            Some(value) => {
                let adj_list: AdjacencyList = bincode::deserialize(&value)
                    .context("Failed to deserialize adjacency list")?;
                Ok(Some(adj_list))
            }
            None => Ok(None),
        }
...
mod tests {
    ...
    fn test_subgraph_query() {
        ...
        storage.upsert_edge(Edge::new(gandalf.id, frodo.id, EdgeType::Knows)).unwrap();
        storage.upsert_edge(Edge::new(frodo.id, sam.id, EdgeType::AllyOf)).unwrap();
