...
impl<'a> DataIngestion<'a> {
    ...
    pub async fn import_json_data<P: AsRef<Path>>(
        &mut self,
        data_file: P,
    ) -> Result<()> {
        ...
        info!("Parsed {} nodes and {} edges from JSON", nodes.len(), edges.len());
    ...
    /// Import data using default paths
    /// 
    /// Uses environment variable if set:
    /// - UFORGE_DATA_FILE: Path to JSON data file
    pub async fn import_default_data(&mut self) -> Result<()> {
        // Get data file from environment or use defaults
        let data_file = std::env::var("UFORGE_DATA_FILE")
            .unwrap_or_else(|_| self.get_default_data_path());
            
        if std::path::Path::new(&data_file).exists() {
            self.import_json_data(&data_file).await?;
        } else {
            return Err(anyhow::anyhow!("Data file not found: {}", data_file));
        }
        
        Ok(())
    }

    /// Get default data file path with fallbacks
    fn get_default_data_path(&self) -> String {
        let data_paths = [
        ...
        "./examples/data/memory.json".to_string() // Default fallback
    }
    ...
    async fn create_objects(
        ...
        for entry in nodes {
            if let JsonEntry::Node { name, node_type, metadata } = entry {
                let object_metadata = self.create_object_by_type(&name, &node_type, &metadata).await?;
    ...
    async fn create_relationships(
        ...
        info!("Created {} relationships total", self.stats.relationships_created);
        Ok(())
    }
...
{"type": "edge", "from": "Location A", "to": "Location B", "edgeType": "connects_to"}"#;
    ...
    async fn test_metadata_parsing() {
        ...
        assert_eq!(object.properties.get("key").and_then(|v| v.as_str()), Some("value"));
        assert_eq!(object.properties.get("another").and_then(|v| v.as_str()), Some("complex value"));
    }
}