...
1. **Env vars are overrides, not requirements.** `cargo run --manifest-path crates/u-forge-core/Cargo.toml --example cli_demo` must work with zero env vars set. Always try `http://localhost:13305/api/v1` first.
2. **Fetch live state before assuming capabilities.** Use `SystemInfo::fetch()` and `LemonadeServerCatalog::discover()`.
