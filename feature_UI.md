...
10. ✅ **Node detail panel** — `NodeDetailPanel` entity replaces the editor placeholder (top 30% of workspace). Observes `SelectionModel`; shows "Select a node to view details" when empty. Selected state renders a tab bar ("Overview") + scrollable pretty-printed JSON: `id`, `name`, `type`, `properties`. `properties` merges the dedicated `description` column (shown first), all `properties` JSON blob fields, and `tags` (from properties or fallback column) — deduplicating across both storage locations. Long string values are word-wrapped at 72 columns with continuation lines aligned under the opening `"`.
